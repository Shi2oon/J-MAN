...
% [xo,yo] = ginput(2);
X = unique(mesh.UDIC(1,:));         Y = unique(mesh.UDIC(2,:));
xo=[X(2); X(end-1)];                yo=[Y(2); Y(end-1)];
...
% [xi,yi] = ginput(2);
if      max(xmask) >= length(X)-10;     maxxmask = min(xmask);  
else;   maxxmask = max(xmask);          end

xi = [X(maxxmask-2) X(maxxmask+2)];     yi = [Y(min(ymask)-1) Y(max(ymask)+1)];
