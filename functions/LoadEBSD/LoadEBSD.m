function [alldata,var] = LoadEBSD(var)
...
SavingD = fullfile(var.results,[var.fillname '.' num2str(var.Trail_number)]);
...
var.Save = [SavingD '_Croped Maps.png'];    saveas(gcf,var.Save); 
var.Save = [SavingD '_Croped Maps.fig'];    saveas(gcf,var.Save); close all
...
var.S23 =  Maps.crop.S{2,3};       var.S33 =  Maps.crop.S{3,3};
...
var.Dir =  SavingD;                var.Stiffness  =  Maps.Stiffness*1e9; %in Pa
var.nu  =  var.Stiffness(1,2)/(var.Stiffness(1,1)+var.Stiffness(1,2));
var.E   =  var.Stiffness(1,1)*(1-2*var.nu)*(1+var.nu)/(1-var.nu); 

%% Save Data  fo J-MAN
alldata = [Maps.crop.Xsc(:)    Maps.crop.Ysc(:)    Maps.crop.E{1,1}(:) Maps.crop.E{2,2}(:)...
            Maps.crop.E{1,2}(:) Maps.crop.S{1,1}(:) Maps.crop.S{2,2}(:) Maps.crop.S{1,2}(:)];
alldata(isnan(alldata)) = 0;% Replace NaN values with 0
Maps     = var; 
save([SavingD '_Data.mat'],'Maps','alldata');
end